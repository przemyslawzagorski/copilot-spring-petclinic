...
Zestaw uruchamialnych rozwiązań do ćwiczeń **ex_25..ex_30** plus **realne** testy
...
```
solutions/
├─ _common.py                       # lokalizacja root repo
...
├─ ex_30_live_demo/ai_server.py     # ex_30 – FastAPI + Spring + custom tools
├─ tests/test_solution_imports.py   # offline – importy i kontrakty bez autoryzacji
...
SDK 1.x zarządza zgodnym runtime Copilot CLI. Przed szkoleniem pobierz go jawnie,
aby uniknąć pobierania przy pierwszym ćwiczeniu:
...
a jeśli ustawisz `COPILOT_GITHUB_TOKEN` — token ma priorytet i jest przekazywany
do podprocesu CLI.
...
### Metoda B — `COPILOT_GITHUB_TOKEN` (do CI / automatów)
...
```powershell
$env:COPILOT_GITHUB_TOKEN = "<twoj_token>"
python .\solutions\ex_25_setup\smoke_test.py
```

> ⚠️ Nie commituj tokena. Nie loguj `$env:COPILOT_GITHUB_TOKEN`.
...
- Łączny czas oczekiwany ~3–5 min (pięć testów na modelu `auto`).

Co weryfikujemy:

| Test | Co sprawdza |
|------|-------------|
| `test_ex25_list_models_nonempty` | `client.list_models()` zwraca >0 i zawiera alias `auto` |
...
|---------|-----------|---------|
| `JsonRpcError -32603: Not authenticated` | Brak sesji w CLI i nieustawiony `COPILOT_GITHUB_TOKEN` | Metoda A albo B z sekcji 2. |
| Runtime nie startuje | Runtime nie został pobrany lub blokuje go proxy/antywirus | Uruchom `python -m copilot download-runtime`. |
...
- Nie commituj `COPILOT_GITHUB_TOKEN`. Trzymaj w sekrecie CI / `.env` ignorowanym przez
  Git.
